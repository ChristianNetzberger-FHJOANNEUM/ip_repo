...
architecture rtl of circuit_ch2_3c is
  ...
  signal timer_shift: std_logic_vector(27 downto 0):= (others => '0'); -- init to zero for simulation
  signal timer_shift_tc: std_logic; --terminal count for timer
...
--buttons_q2 is the synchronized and debounced signal
--buttons_q3 is just needed for edge detection after the debounce flipflop

  --timer_shift to generate a 100ms strobe for shifting--
  process(clk)
  begin
    if rising_edge(clk) then
        if timer_shift_tc = '1' then
           timer_shift <= (others => '0');
         else
           timer_shift <= timer_shift+1;
        end if;
      end if;
  end process;

-- compute the terminal count signal for the timer         
   timer_shift_tc <= '1' when timer_shift = 12500000-1 else '0'; -- no fulladder inferred
                                                   -- 12500000-1 is computed at compile time
  ...
  begin
    if rising_edge(clk) then
      if sliders_q2(0) = '1' then  ---synchronized signal slider SW0
        shreg <= (others => '0');
      elsif timer_shift_tc = '1' then
         if sliders_q2(1) = '1' then  ---synchronized signal slider SW1
           shreg <= buttons_q2(0) & shreg(shreg'left downto 1); -- shift in state of BT0
         else
           shreg <= shreg(shreg'left-1 downto 0) & buttons_q2(0); -- shift in state of BT0
         end if;
      end if;
    end if;
  end process;
